...
6. [`20260819140000_quien_dio_la_clase.sql`](migrations/20260819140000_quien_dio_la_clase.sql)
...
clase tenga VARIOS docentes a cargo a la vez. La sexta agrega el registro de quién
dio la clase CADA FECHA, para poder anotar las suplencias sin cambiar el horario.

> La quinta y la sexta hay que aplicarlas **antes** de publicar la app, no después:
> la pantalla lee `clase_docentes` y `clases_dictadas` desde el arranque y sin ellas
> no carga nada. Las asignaciones que ya existían se conservan solas.
...
1. Copiá la migración que corresponda entera al portapapeles. Para la nueva, en
   ...
   ```powershell
   Get-Content "supabase\migrations\20260819140000_quien_dio_la_clase.sql" -Raw | Set-Clipboard
   ```
...
**Cómo saber que quedó bien:** andá a **Table Editor**. Tienen que estar las nueve
tablas — `clientes`, `clases`, `participantes`, `pagos`, `asistencias`, `docentes`,
`clase_docentes`, `clases_dictadas`, `lista_espera` — y cada una
